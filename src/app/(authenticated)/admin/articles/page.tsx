...
export default function ArticlesPage() {
  const { articles, deleteArticle, bulkUpdateStatus, bulkDelete } = useAdmin();
  ...
  const handleBulkDelete = async () => {
    try {
      await bulkDelete("article", selectedIds);
      toast.success(`${selectedIds.length} articles deleted`, {
        description: "The selected articles have been removed.",
      });
      setSelectedIds([]);
    } catch (error) {
      ...
      toast.error("Error deleting articles", {
        description:
          "There was a problem deleting the articles. Please try again.",
      });
    }
  };
  ...
  const handleBulkStatusChange = async (status: ContentStatus) => {
    try {
      await bulkUpdateStatus("article", selectedIds, status);
      toast.success(`Status updated`, {
        description: `${selectedIds.length} articles marked as ${status}.`,
      });
      setSelectedIds([]);
    } catch (error) {
